...
class plc5:
	...
	def Pre_Main_High_Pressure_RO(self,IO,HMI,Sec_P,Min_P):
		...
		self.Mid_FIT501_Tot_Enb	= HMI.P501.Status==2 or HMI.P502.Status==2  # P501 or P502 starts => FIT501 can be enable
		self.Mid_FIT502_Tot_Enb	= HMI.MV501.Status==2                       # MV501 opens (RO Permeate) => FIT502 can be enable
		self.Mid_FIT503_Tot_Enb	= HMI.P401.Status==2 or HMI.P402.Status==2  # P401 or P402 starts => FIT503/4 can be enable
		...
		HMI.P502.Permissive[0] = HMI.P_RO_FEED_DUTY.Pump_Running            # start P401 or P402
...
#Main
		...
		while switch(HMI.P5.State):
			...
			if case(2):
				...
				HMI.Cy_P5.MV503_TIMEOUT_TM = 0
				HMI.Cy_P5.MV504_TIMEOUT_TM = 0
				if HMI.P_RO_FEED_DUTY.Pump_Running and self.Mid_NEXT:    # If Start P401/2
					self.Mid_NEXT = 0
					HMI.P5.State = 3
				break
			if case(3):
				...
				self.Mid_MV501_AutoInp = 0
				self.Mid_MV502_AutoInp = 0
				self.Mid_MV503_AutoInp = 1                                # Open MV503
				self.Mid_MV504_AutoInp = 1                                # Open MV504
				self.Mid_P50X_AutoSpeed	= HMI.Cy_P5.VSD_MIN_SPEED
				HMI.Cy_P5.RO_HPP_SD_On = 0
				HMI.Cy_P5.FLUSHING_MIN = 0
				HMI.Cy_P5.RO_SD_FLUSHING_MIN = 0
				...
				HMI.Cy_P5.MV503_TIMEOUT_TM = 0
				HMI.Cy_P5.MV504_TIMEOUT_TM = 0
				if HMI.MV503.Status==2 and HMI.MV504.Status==2 and HMI.P_RO_FEED_DUTY.Pump_Running or self.Mid_NEXT: # If open MV502 and MV504 and start P401/2
					self.Mid_NEXT = 0
					HMI.P5.State = 4
				break
			if case(4):
				...
				HMI.Cy_P5.MV503_TIMEOUT_TM = 0
				HMI.Cy_P5.MV504_TIMEOUT_TM = 0
				# if Min_P:                                                          # Loop flushing procedure and time the procedure
				...
				# 	self.Mid_NEXT = 0
				# 	HMI.P5.State = 5
				# 	print("Flushing procedure done")
				break
			if case(5):
				self.Mid_P_RO_HIGH_AutoInp = 1                                     # Start P501/2
				self.Mid_MV501_AutoInp = 0
				self.Mid_MV502_AutoInp = 0
				self.Mid_MV503_AutoInp = 1
				self.Mid_MV504_AutoInp = 1
				self.Mid_P50X_AutoSpeed	= HMI.Cy_P5.VSD_MIN_SPEED # 10
				HMI.Cy_P5.RO_HPP_SD_On = 0
				HMI.Cy_P5.FLUSHING_MIN = 0
				HMI.Cy_P5.RO_SD_FLUSHING_MIN = 0
				HMI.Cy_P5.RO_HIGH_PUMP_Shutdown	= 0
				...
				HMI.Cy_P5.MV503_TIMEOUT_TM = 0
				HMI.Cy_P5.MV504_TIMEOUT_TM = 0
				if (HMI.P501.Speed >= self.Mid_P50X_AutoSpeed) or (HMI.P502.Speed >= self.Mid_P50X_AutoSpeed) or self.Mid_NEXT: # Default HMI.P501.Speed=0
					self.Mid_NEXT = 0
					HMI.P5.State = 6
				break
			if case(6):
			...
			if case(16):
				...
				if HMI.Cy_P5.MV502.Status==1 or self.Mid_NEXT or HMI.Cy_P5.MV502_TIMEOUT_TM > 120:
					self.Mid_NEXT = 0
					HMI.P5.State = 17
				break
			if case(17):
				...
				if HMI.Cy_P5.MV503.Status==2 or self.Mid_NEXT or HMI.Cy_P5.MV503_TIMEOUT_TM > 120 :
					self.Mid_NEXT = 0
					HMI.P5.State = 18
				break
			if case(18):
			...
			if case(19):
				...
				HMI.Cy_P5.MV504_TIMEOUT_TM = 0
				if Min_P:
					HMI.Cy_P5.RO_SD_FLUSHING_MIN = HMI.Cy_P5.RO_SD_FLUSHING_MIN + 1
			...
			if case(21):
				...
				if Sec_P:
					HMI.Cy_P5.MV503_TIMEOUT_TM = HMI.Cy_P5.MV503_TIMEOUT_TM + 1
					HMI.Cy_P5.MV504_TIMEOUT_TM = HMI.Cy_P5.MV504_TIMEOUT_TM + 1
				if HMI.MV503.Status==1 and HMI.MV504.Status==1 or self.Mid_NEXT or HMI.MV503_TIMEOUT_TM >120 or HMI.MV504_TIMEOUT_TM >120 :
					HMI.P5.State = 1
				break
			HMI.P5.State=1
			break

		#print("P5.State: {}".format(HMI.P5.State))
