...
# there could be CIP in the future, but current SWaT system has none
	def __init__(self,HMI):
		self.LIT301_FB = AIN_FBD(HMI.LIT301)                                                   # UF Feedwater Tank
		...
		self.PSH301_FB = SWITCH_FBD(HMI.PSH301)                                                # Pressure Sensor High
		self.DPSH301_FB = SWITCH_FBD(HMI.DPSH301)                                              # Differential Pressure Sensor High
		self.DPIT301_FB = AIN_FBD(HMI.DPIT301)                                                 # Differential Pressure Indicator Transmitter
		...
		self.Mid_NEXT = 0                                                                      # Whether directly jump to next state
	...
	def Pre_Main_UF_Feed(self, IO, HMI, Sec_P, Min_P):
		...
		# Related pump starts but water flow is super low
		...
		HMI.P301.Permissive[2] = HMI.MV302.Status==2 or HMI.MV304.Status==2 # UF flushing procedure or UF ultra filtration procedure
...
#	def Main_Seq(self,Sec_P,Min_P):
		...
		if HMI.DPIT301.Hty:   # TMP_High represent differential pressure condition
			HMI.PLANT.TMP_High = HMI.DPIT301.AH
		else:
			HMI.PLANT.TMP_High = HMI.DPSH301.Alarm

		if HMI.LIT401.AH and HMI.P3.State > 1:  # If tank401 is high, then only UF flushing procedure
			HMI.P3.State = 99

		#(*-----------WRITE FAULT CONDITION--------------*)
		while switch(HMI.P3.State):  # Reset UF cycle parameters when state=1
			if case(1):
			...
			if case(2):
				...
				self.Mid_MV302_AutoInp = 0
				self.Mid_MV303_AutoInp = 0
				self.Mid_MV304_AutoInp = not HMI.LIT301.ALL and not HMI.LIT401.AH  # Open MV304
				self.Mid_P_UF_FEED_DUTY_AutoInp = 0
				self.Mid_P602_AutoInp = 0
				self.Mid_P_NAOCL_UF_DUTY_AutoInp = 0
				HMI.Cy_P3.UF_REFILL_SEC	= 0
			...
			if case(3):
				...
				self.Mid_MV302_AutoInp = 0
				self.Mid_MV303_AutoInp = 0
				self.Mid_MV304_AutoInp = 1                                         # Open MV304
				self.Mid_P_UF_FEED_DUTY_AutoInp	= 1                                # Start P301/2
				self.Mid_P602_AutoInp = 0
				self.Mid_P_NAOCL_UF_DUTY_AutoInp = 0
				HMI.Cy_P3.UF_REFILL_SEC	= 0
				HMI.Cy_P3.UF_FILTRATION_MIN	= 0
			...
			if case(4):
				...
				HMI.Cy_P3.CIP_CLEANING_SEC = 0
				HMI.Cy_P3.DRAIN_SEC	= 0
				if Sec_P:                                                          # Loop UF flushing procedure and time the procedure:30s
					HMI.Cy_P3.UF_REFILL_SEC = HMI.Cy_P3.UF_REFILL_SEC + 1
					if HMI.Cy_P3.UF_REFILL_SEC > HMI.Cy_P3.UF_REFILL_SEC_SP or self.Mid_NEXT: # If UF flushing procedure reach its cost time then jump to next state
						self.Mid_NEXT = 0
						HMI.P3.State = 5
						#print("UF flushing procedure done")
				break
			if case(5):
				self.Mid_Last_State = HMI.P3.State
				self.Mid_MV301_AutoInp = 0
				self.Mid_MV302_AutoInp = 1                                         # Open MV302
				self.Mid_MV303_AutoInp = 0
				self.Mid_MV304_AutoInp = 1                                         # Open MV304
				self.Mid_P_UF_FEED_DUTY_AutoInp	= 1                                # Start P301/2
				self.Mid_P602_AutoInp = 0
			...
			if case(6):
				...
				self.Mid_MV302_AutoInp = 1                                         # Open MV302
				self.Mid_MV303_AutoInp = 0
				self.Mid_MV304_AutoInp = 0                                         # Close MV304
				self.Mid_P_UF_FEED_DUTY_AutoInp	= 1                                # Start P301/2
				self.Mid_P602_AutoInp = 0
				self.Mid_P_NAOCL_UF_DUTY_AutoInp = 0
				HMI.Cy_P3.UF_REFILL_SEC	= 0
			...
			if case(10):
				self.Mid_Last_State = HMI.P3.State
				self.Mid_MV301_AutoInp = 1                                         # Open MV301
				self.Mid_MV302_AutoInp = 0
				self.Mid_MV303_AutoInp = 1                                         # Open MV303
				self.Mid_MV304_AutoInp = 0
				self.Mid_P_UF_FEED_DUTY_AutoInp	= 0
				self.Mid_P602_AutoInp = 0
				self.Mid_P_NAOCL_UF_DUTY_AutoInp = 0
			...
			if case(11):
				...
				self.Mid_MV304_AutoInp = 0
				self.Mid_P_UF_FEED_DUTY_AutoInp	= 0
				self.Mid_P602_AutoInp = 1                                          # Start P602
				self.Mid_P_NAOCL_UF_DUTY_AutoInp = 0
				HMI.Cy_P3.UF_REFILL_SEC	= 0
				HMI.Cy_P3.UF_FILTRATION_MIN	= 0
				HMI.Cy_P3.BACKWASH_SEC = 0
			...
			if case(16):  
				...
				if Sec_P:
					...
					if HMI.Cy_P3.DRAIN_SEC > HMI.Cy_P3.DRAIN_SEC_SP or self.Mid_NEXT: # UF feed tank draining procedure
						self.Mid_NEXT = 0
						HMI.P3.State = 4                                              # Jump to state=4 Loop UF flushing procedure
						#print("UF feed tank draining procedure done")
				break
			if case(17):                                                              # No CIP
				self.Mid_MV301_AutoInp = 1
				self.Mid_MV302_AutoInp = 0
			...
			if case(18):      
				...
				self.Mid_P_UF_FEED_DUTY_AutoInp	= 0
				self.Mid_P602_AutoInp = 0
				self.Mid_P_NAOCL_UF_DUTY_AutoInp = 1                               # Open P207/8
				HMI.Cy_P3.UF_REFILL_SEC	= 0
				HMI.Cy_P3.UF_FILTRATION_MIN	= 0
				HMI.Cy_P3.BACKWASH_SEC = 0
				HMI.Cy_P3.DRAIN_SEC	= 0
		...
		HMI.Mid_P602_AutoInp = self.Mid_P602_AutoInp #(*SIGNAL TO P6)
