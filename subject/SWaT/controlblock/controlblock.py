...
class FIT_FBD:
	...
	#
	# 	# Whether sensor return value is in the defined low and high bounds
	...
	# 		HMI.Pv = HMI.Sim_Pv
	# 	else:
	# 		# Compute HMI value that corresponding to sensor return value
	...
	# 	self.AHH, self.AH, self.AL, self.ALL = ALM(HMI.Pv,SAHH,SAH,SAL,SALL)
	#
	# 	# Water volume each second pulse (m^3)
	# 	if Totaliser_Enb:
	# 		if Sec_P:
...
class PMP_FBD:
	def __init__(self, HMI):
		...
		self.SD = bit_2_signed_integer(HMI.Shutdown)   # Initialize shutdown conditions

		self.Cmd_Start = 0                             # Initialize pump command (start/stop)
		self.RunMin = 0
		self.Total_RunMin = 0
			...
			if not Auto:
				...
				if AutoInp:
					if not self.Fault and Permissive == -1 and bit_2_signed_integer(HMI.SD)== 0:
						...
						self.SD = bit_2_signed_integer(HMI.SD) # SD!=0 => Update it
						self.Cmd_Start = 0
				else:
					self.Cmd_Start = 0
		else:
...
class VSD_FBD:
	...
	def VSD_FBD(self, AutoInp, AutoSpeed, VSD_In, VSD_Out, IO, HMI):
		...
		if AutoInp:
			if not VSD_In.Faulted and Permissive == -1 and bit_2_signed_integer(HMI.SD) == 0:
				VSD_Out.Start = 1
				VSD_Out.Stop = 0
